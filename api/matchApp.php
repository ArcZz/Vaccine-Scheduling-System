...
$unassignedPatient = $db->get_results('SELECT pa_id, p_number, max_travel_distance, pa_address
                                  FROM Patient
                                  WHERE pa_id NOT IN (SELECT pa_id
					                                             FROM AppOffer
  ...
  for($i = 0; $i < count($paArrayGroup); $i++){
    ...
    for($j=0; $j < count($appArray); $j++){
      ...
      // echo "pr_lon: ".$pr_lon.'<br/>';
       ...
       // echo "max distance: ".$paArrayGroup1[$i]->max_travel_distance.'<br/>';

      if($distance <= $paArrayGroup[$i]->max_travel_distance){
